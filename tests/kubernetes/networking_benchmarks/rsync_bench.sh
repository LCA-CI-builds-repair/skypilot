...
output_file="rsync_results_${suffix}.txt"
...
if [ "$runexpt" -eq 1 ]; then
  ...
  # Setup 1000 1MB (total 1GB) files to copy
  mkdir -p $HOME/tmp/uploadtest
  for i in {1..1000}; do dd if=/dev/urandom of=$HOME/tmp/uploadtest/$i bs=1M count=1; done
  ...
  for i in {1..5}; do
      (
          ...
          time rsync -avz ~/tmp/uploadtest/ test:~/sky_workdir
      ) 2>> "$output_file"
      ssh test 'rm -rf ~/sky_workdir/'
  done

  # Delete cluster after done
  sky down -y test
fi
