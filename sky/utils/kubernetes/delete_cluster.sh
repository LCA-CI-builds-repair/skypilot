...
if ! docker info > /dev/null 2>&1; then
    >&2 echo "Error: Docker is not running. Please start Docker and try again."
    exit 1
fi

# Check if kind is installed
if ! kind version > /dev/null 2>&1; then
    >&2 echo "Error: kind is not installed. Please install kind and try again."
    exit 1
fi
...
if kind get clusters | grep -q skypilot; then
    ...
    echo "Local cluster does not exist. No action needed."
fiocal kind cluster
# Usage: ./delete_cluster.sh
# Raises error code 100 if the local cluster does not exist
